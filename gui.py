...
def _parse_base_addresses(base_text):
    ...
    for part in parts:
        ...
        # 支持紧凑写法：4001B/1D -> 4001B, 4001D
        if len(p) < len(first):
            p = first[: len(first) - len(p)] + p

        value = int(p, 16)
        # 例如 4001B/4001D -> 0x4001B000 / 0x4001D000
        if len(p) <= 5:
            value <<= 12
        addrs.append(value)
    return addrs
...
def _get_isp_csv_candidates():
    return [
        os.path.join(os.path.dirname(__file__), "ISP.csv"),
    ]
...
class FpgaGui(tk.Tk):
    def __init__(self):
        ...
        self.reg_value_vars = {}  # addr -> StringVar
        self.reg_entry_widgets = {}  # addr -> Entry
        self.func_card_vars = {}  # module -> field_name -> StringVar
        ...
        self.fixed_row_items = []  # index -> register dict
        self.reg_rows = []  # [{"name":..., "widgets":[...]}]
